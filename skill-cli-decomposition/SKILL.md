...
| 字段 | 必需 | 类型 | 说明 |
|------|------|------|------|
| item_id | 是 | string | 全局唯一标识，如 `item-1-1` |
| title | 是 | string | 子任务标题 |
| description | 否 | string | 详细描述 |
| required_skills | 否 | string[] | 技能标签 |
| depends_on | 是 | string[] | 依赖的 item_id 列表（允许空列表） |
| assignee_role | 否 | string | 执行者角色 |
| type | 否 | string | 任务类型（默认 `other`） |
| order | 否 | number | 执行顺序（从 0 开始） |
