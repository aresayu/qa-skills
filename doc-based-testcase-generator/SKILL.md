...
description: 基于需求文档、PRD或接口文档自动生成结构化测试用例文档。默认采用通用测试用例设计策略（正向、反向、边界值、等价类等）；当用户提到参考 Excel/Word 模板时从 assets 加载模板，提到接口/性能/功能等专用标准时从 references 加载对应说明。适用于从各类文档设计功能、接口、性能及自动化候选用例。
---
...
- **输出组织结构（总-分结构，必须遵循）**
  默认以「总文档 + 分模块文档」组织输出，职责单一、便于维护与导航：
  - **总文档 `INDEX.md`**：索引导航 + 公共信息（认证、通用响应、错误码、安全规范）。
  - **分文档（按业务模块）**：各模块独立成文，仅包含该模块相关测试用例与模块特有说明。

  当用户未指定目录结构时，默认按以下目录输出（适用于“接口/API 测试用例”类产物；如用户明确要求功能/性能/自动化用例，也沿用该“总-分”结构，但类型根目录名称随测试类型变化）：
  ...
  - **模块划分原则**：按文档中的业务域/菜单/子系统划分；若用户提供了模块清单，严格按用户清单分组；若未提供，则先从文档中抽取模块并在输出的 `INDEX.md` 中列明模块列表与口径。
  - **编号与命名**：顶层模块目录采用两位数前缀（10/20/30...）确保排序稳定；模块内用例文件统一命名为 `<编号>-测试用例.md`；概述模块固定为 `10-概述/10-测试用例概述.md`。
  - **内容分配规则**：
    - `INDEX.md`：仅放索引与公共信息，不堆叠各模块详细用例。
    - `10-测试用例概述.md`：测试范围、测试类型说明、通用字段口径、通用前置条件/数据准备、通用约束（含通用错误码/响应结构引用）、执行与回归策略（如适用）。
    - 各模块 `xx-测试用例.md`：该模块的用例清单（可按功能点/接口分组），每条用例需体现：用例标识、标题、所属模块/接口、用例类型、优先级、前置条件、步骤、预期结果，以及可选的数据要求/备注。

- **格式与列名**
  SKILL 不强制具体表格列名或 Markdown 表格样式，但必须满足：
  - 默认运用第三节的通用测试用例设计策略；
  - 输出中需能体现：用例标识、标题、所属模块/接口、用例类型、优先级、前置条件、步骤、预期结果，以及可选的数据要求/备注；
  - 具体列名、顺序与排版以 references 标准或 assets 模板为准；若与“总-分结构”冲突，以“总-分结构”的文件拆分优先，再在各文件内按模板/标准排版。
...
- **默认行为**：生成的测试用例文档（以“总-分结构”展示：`INDEX.md` + 各模块文件的内容）。**必须**保存到目标路径下的 `testcase/` 目录，并在其内部按文档中描述的“总-分结构”创建与写入文件。
  ...
  - `testcase/` 内的类型根目录：接口类默认 `api/`；若用户明确是功能/性能/自动化用例，可分别使用 `functional/`、`performance/`、`automation/`，但**内部结构仍需遵循“总-分结构”**（包含 `INDEX.md` 与按模块拆分的子目录/文件）。
